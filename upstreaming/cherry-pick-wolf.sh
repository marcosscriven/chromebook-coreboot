...
# This a script to cherry pick commits for the Wolf mainboard
# The intention is to get everything we need from Google committed into coreboot upstream
# I can then submit my own patches to coreboot to make the necessary to changes
...
#
# If I rebased the Wolf branch to fix this, cherry picks would have meaningless hashes local to my machine
# Instead I'm removing these dirs and committing, with the intention of removing that commit by rebasing.
git rm 3rdparty/blobs 3rdparty/vboot
git commit -am "--- DO NOT MERGE - This is a temporary local commit. ---"

# Record the commit to remove after we've cherry picked
...
# In any case, these don't touch src/mainboard/google/wolf

# I'm hoping at this point someone with admin rights can push this
